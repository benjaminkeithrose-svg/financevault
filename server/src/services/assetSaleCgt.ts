 ...
 * improvements + selling costs. Each owner's share of the gain goes to that
...
export function saleGains(asset: SoldAsset, entities: Map<string, { name: string; entityType: string }>): SaleGainRow[] {
  ...
  const costBase = (purchase ?? 0) + (asset.buyingCosts ?? 0) + (asset.improvementsCost ?? 0) + (asset.sellingCosts ?? 0);
