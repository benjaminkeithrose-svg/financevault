# Source documents: received and still needed

Generated from link-pack.json and sources/index.json. 37 of 46 documents saved (index pages don't need saving).

## Still needed

Open each link on your own computer, save it as a PDF (for web pages use the page's "Print" or "Print whole section" option, then "Save as PDF"), and upload it.

1. **PCG 2026/2 — apportioning rental property deductions (part-private use)**
   https://www.ato.gov.au/law/view/document?DocID=COG/PCG20262/NAT/ATO/00001
2. **Rental properties guide (yearly)**
   https://www.ato.gov.au/api/public/content/d1dd11d5fc1a460f9b4b6faf74e79b4a?v=c79af85b
3. **Guide to depreciating assets (yearly)**
   https://www.ato.gov.au/api/public/content/b5c7bb178b9f4757bd7a2b1d82c119c1?v=5f0342dc
4. **Effective life of depreciating assets — determinations, rulings and law**
   https://www.ato.gov.au/businesses-and-organisations/income-deductions-and-concessions/depreciation-and-capital-expenses-and-allowances/general-depreciation-rules-capital-allowances/effective-life-of-an-asset/effective-life-determinations-rulings-and-law
5. **Guide to capital gains tax (yearly)**
   https://www.ato.gov.au/api/public/content/359216df15a44556a9af2ac851c1ef5a
6. **Medicare levy**
   https://www.ato.gov.au/individuals-and-families/medicare-and-private-health-insurance/medicare-levy
7. **Occupation guides — 'print whole section' (all guides in one)** - What downloaded was only the list of guides. Instead, open the guide for each occupation you need (from the list) and use its "Print whole section" option.
   https://www.ato.gov.au/api/public/content/0-23394288-dde4-4157-950f-33536a6bb202
8. **FBT on cars, other vehicles, parking and tolls**
   https://www.ato.gov.au/businesses-and-organisations/hiring-and-paying-your-workers/fringe-benefits-tax/types-of-fringe-benefits/fbt-on-cars-other-vehicles-parking-and-tolls
9. **FBT electric cars exemption (novated leases included)**
   https://www.ato.gov.au/law/view/pdf/afs/afs_ev_fbt.pdf
10. **ASIC RG 209 — responsible lending conduct** - Only the web landing page was received. The guide itself is the PDF behind its 'Download' button, hosted on download.asic.gov.au.
   https://www.asic.gov.au/regulatory-resources/find-a-document/regulatory-guides/rg-209-credit-licensing-responsible-lending-conduct

## Received

- `tr-2000-2.pdf` - covers: tr-2000-2
- `tr-95-25.pdf` - covers: tr-95-25
- `td-2012-1.pdf` - covers: td-2012-1
- `tr-97-23.pdf` - covers: tr-97-23
- `rental-expenses.pdf` - covers: rental-interest-expenses - Whole 'Rental expenses' section (73 pages): interest, borrowing expenses, repairs, capital works, depreciating assets.
- `smsfr-2012-1.pdf` - covers: smsfr-2012-1 - ATO says it is reviewing this ruling because LRBA law changes commence 10 August.
- `pcg-2016-5.pdf` - covers: pcg-2016-5
- `tr-2022-4.pdf` - covers: tr-2022-4 - ATO says this ruling is under review after Commissioner of Taxation v Bendel [2026] HCA 18.
- `pcg-2022-2.pdf` - covers: pcg-2022-2
- `tax-rates-residents.pdf` - covers: tax-rates-residents
- `private-health-rebate.pdf` - covers: private-health-rebate
- `work-related-deductions.pdf` - covers: work-related-deductions, wfh-expenses, wfh-fixed-rate
- `car-own-or-lease.pdf` - covers: car-own-or-lease, car-cents-per-km - Whole 'Motor vehicle and car expenses' section.
- `working-as-an-employee.pdf` - covers: salary-sacrifice, income-statements
- `super-new-legislation.pdf` - covers: super-new-legislation - New super laws from 1 July 2026: Division 296 tax on balances over $3m, payday super, and LRBA borrowing limited to business real property for arrangements from 10 August 2026.
- `nsw-land-tax-rates.pdf` - covers: nsw-land-tax-rates
- `nsw-land-tax-trusts.pdf` - covers: nsw-land-tax-trusts
- `nsw-transfer-duty.pdf` - covers: nsw-transfer-duty
- `apra-apg-223.pdf` - covers: apra-apg-223 - Fetched directly from apra.gov.au.
- `apra-macroprudential.pdf` - covers: apra-macroprudential
- `cgt-personal-investors-guide.pdf` - covers: cgt-personal-investors-guide
- `fbt-rates.pdf` - covers: fbt-rates
- `div7a-benchmark-rate.pdf` - covers: div7a-benchmark-rate
- `self-education.pdf` - covers: self-education - Whole "Education, training and seminars" section.
- `occupation-guides.pdf` - covers: occupation-guides - The ATO print of this section is only the list of guides (6 pages), not the guides themselves. Individual guides are needed for the occupations that matter (e.g. sales and marketing, teachers).
- `income-deductions-offsets-and-records.pdf` - covers: record-keeping - Whole "Income, deductions, offsets and records" section (70 pages), including records you need to keep.
- `income-statements.pdf` - covers: income-statements
- `gstr-2002-5.pdf` - covers: gstr-2002-5
- `medicare-levy-surcharge.pdf` - covers: medicare-levy-surcharge - Whole "Medicare levy surcharge" section.
- `super-rates-thresholds.pdf` - covers: super-rates-thresholds, super-contribution-caps, super-minimum-pension - Whole "Key super rates and thresholds" section (74 pages).
- `lcr-2021-2.pdf` - covers: lcr-2021-2
- `apra-dti-limit.txt` - covers: apra-dti-limit - Fetched directly from apra.gov.au (web page saved as text).
